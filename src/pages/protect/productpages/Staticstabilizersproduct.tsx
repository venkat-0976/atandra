import { CheckCircle, Zap, Shield, Settings, Mail, Star, Download, Info } from 'lucide-react';
...
const StaticStabilizersProduct = () => {
  ...
  const canonicalUrl = location.pathname.includes('three-phase-static-voltage-regulator')
    ? 'https://atandra.in/protect/static-stabilizers/product/three-phase-static-voltage-regulator'
    : 'https://atandra.in/protect/static-stabilizers/product/static-voltage-stabilizer';

  // Determine title based on current path to avoid duplicates (same pattern as other product pages)
  const pageTitle = location.pathname.includes('three-phase-static-voltage-regulator')
    ? 'Three Phase Static Voltage Regulator 10–1000 kVA  | KRYKARD'
    : 'Static Voltage Stabilizer - DSP IGBT SVR | KRYKARD';

  // Determine description for SEO only (meta description) - not displayed in UI
  const pageDescription = location.pathname.includes('three-phase-static-voltage-regulator')
    ? 'Three phase static voltage regulator with DSP IGBT control, 20kHz switching and 10–1000 kVA range for precision industrial power.'
    : 'Industrial static voltage stabilizer with DSP IGBT technology. ±1% accuracy, 20kHz switching for precision voltage regulation.';

  // Determine H1 heading for UI - should be unique for each product variant
  const pageH1 = location.pathname.includes('three-phase-static-voltage-regulator')
    ? 'Three Phase Static Voltage Regulator'
  ...
  const features = [
    {
      icon: <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" />, // match icon size/style
  ...
  // Note: This page covers both Three Phase and Industrial SVR products
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": "Static Voltage Regulator",
    "description": "DSP-controlled static voltage stabilizers with IGBT technology offering ultra-fast correction, ±1% accuracy and high efficiency for industrial applications.",
    "brand": {
      "@type": "Brand",
      "name": "KRYKARD"
    },
    "model": "Static Voltage Regulator",
    "image": "https://atandra.in/Static_stabilizers/W_5-removebg-preview.png",
    "offers": {
      "@type": "Offer",
  ...
  return (
    <>
      <SeoHead
        ...
        keywords="static stabilizer, static voltage regulator, SVR, IGBT stabilizer, DSP controlled stabilizer, PWM voltage regulator, static voltage control, industrial stabilizer, precision voltage regulator, three phase SVR, industrial SVR"
        canonical={canonicalUrl}
        ogImage="/Static_stabilizers/W_5-removebg-preview.png"
        jsonLd={jsonLd}
        preloadImage="/Static_stabilizers/W_5-removebg-preview.png"
      />
      <PageLayout
        hideHero={true}
      ...
      >
        <div className="bg-white font-sans min-h-screen font-open-sans">
          ...
          <div className="w-full bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 text-white relative overflow-hidden">
            ...
            <div className="w-full px-4 sm:px-6 md:px-8 py-6 md:py-8 lg:py-10 relative z-10">
              <div className="max-w-6xl mx-auto text-center">
                <div className="space-y-6 md:space-y-8">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center pt-2 px-4">
                    ...
                    >
                      <Mail className="w-2.5 h-2.5 sm:w-3 h-3 inline mr-2" />
                      Get Quote
                    </button>
                    <a
                    ...
                    >
                      <Download className="w-2.5 h-2.5 sm:w-3 h-3 inline mr-2" />
                      Download Brochure
                    </a>
                  </div>
                </div>
                      ...
                      <ul className="space-y-2 sm:space-y-3">
                        {feature.items.map((item, itemIndex) => (
                          <li key={itemIndex} className="flex items-start">
                            <CheckCircle className="w-2.5 h-2.5 sm:w-3 h-3 text-blue-600 mt-0.5 mr-2 flex-shrink-0" />
                        ...
                        {feature.items.map((item, itemIndex) => (
                          <div key={itemIndex} className="flex items-start">
                            <div className="bg-blue-100 rounded-full p-1 mr-2 sm:mr-3 mt-0.5 flex-shrink-0">
                              <CheckCircle className="w-2.5 h-2.5 sm:w-3 h-3 text-blue-600" />
                            </div>
                            <span className="text-xs sm:text-sm text-black leading-relaxed font-medium">{item}</span>
                          </div>
                        ))}
          ...
          {/* Call to Action Section - Simple Centered Box with Gradient */}
          <div className="w-full flex justify-center items-center mt-16 mb-24">
            <div className="bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 text-white rounded-2xl shadow-lg px-10 py-10 max-w-2xl w-full text-center">
              <h2 className="text-2xl sm:text-3xl font-bold mb-3">Ready to Upgrade Your Power Protection?</h2>
              ...
              <button
                ...
                className="bg-white text-blue-900 px-6 py-3 rounded-lg font-semibold text-sm hover:bg-blue-50 transition-all duration-300 shadow-md flex items-center justify-center mx-auto"
              >
